...
function DailySpendChart({ months, cur, hidden = [], onOpenRange }) {
  ...
  for (const { key, stmt } of months) {
    ...
    for (let d = 1; d <= dim; d++) {
      ...
      dayLabel.push(months.length > 1 ? window.monthAbbr(key) + " " + d : d + ".");
    }
  ...
  useEffect(() => { setSel(null); setFocus(null); }, [months.map((m) => m.key).join(","), hidden.join(",")]);
  ...
  const onBrushDown = (e) => {
    ...
    const move = (ev) => {
      ...
      if (st.mode === "new") {
        ...
        if (b - a >= 1) setFocus({ a, b });
      } else if (st.mode === "a") {
        setFocus({ a: Math.min(c, st.fB - 1), b: st.fB });
      } else if (st.mode === "b") {
        setFocus({ a: st.fA, b: Math.max(c, st.fA + 1) });
      } else {
  ...
  return (
    ...
    {totalCols > 3 && (
      <div className="dsp-brush">
        <div className="dsp-brush-track" ref={trackElRef} onPointerDown={onBrushDown}
          title="Drag to focus a period · drag the edges to resize · drag the middle to pan">
        ...
        <div className="dsp-brush-side">
          ...
          {focus
            ? <button className="dsp-brush-reset" onClick={() => setFocus(null)} title="Show the whole range">Reset</button>
            : <span className="dsp-brush-hint">drag to focus</span>}
        </div>
      </div>
    )}
