...
function StrategiePanel({ currency, onCurrency, onToast }) {
  ...
  const [ladder, setLadder] = useState(S.ladder);
  const [openRung, setOpenRung] = useState(null);
  const editRung = (id, patch) => setLadder((prev) => prev.map((r) => (r.id === id ? { ...r, ...patch } : r)));
      ...
      <div className="strat-grid">
        ...
        <section className="card span-4">
          <div className="card-head">
            ...
            <span className="rung-hint">tap a step to edit</span>
          </div>
          <ol className="ladder">
            {ladder.map((t) => {
              const isOpen = openRung === t.id;
              return (
              <React.Fragment key={t.id}>
              <li className={"rung rung--" + t.status + (isOpen ? " rung--open" : "")}
                onClick={() => setOpenRung(isOpen ? null : t.id)} role="button" aria-expanded={isOpen}>
                ...
                <SI.chev className="rung-caret" />
              </li>
              {isOpen && (
                <li className="rung-edit" onClick={(e) => e.stopPropagation()}>
                  <div className="rung-edit-grid">
                    <div className="rung-edit-field">
                      <span className="rung-edit-lab">Step</span>
                      <input className="fld fld--sub" value={t.title} onChange={(e) => editRung(t.id, { title: e.target.value })} />
                    </div>
                    <div className="rung-edit-field">
                      <span className="rung-edit-lab">Note</span>
                      <input className="fld fld--sub" value={t.note} onChange={(e) => editRung(t.id, { note: e.target.value })} />
                    </div>
                    <div className="rung-edit-field">
                      <span className="rung-edit-lab">Status</span>
                      <div className="seg">
                        {[["done", "Done"], ["active", "Active"], ["queued", "Queued"]].map(([id, lab]) => (
                          <button key={id} className={"seg-btn" + (t.status === id ? " on" : "")}
                            onClick={() => editRung(t.id, { status: id, pct: id === "active" ? (typeof t.pct === "number" ? t.pct : 50) : t.pct })}>{lab}</button>
                        ))}
                      </div>
                    </div>
                    {t.status === "active" && (
                      <div className="rung-edit-field">
                        <span className="rung-edit-lab">Progress</span>
                        <div className="rung-edit-row">
                          <input type="range" className="slider" min="0" max="100" step="5"
                            value={typeof t.pct === "number" ? t.pct : 0}
                            onChange={(e) => editRung(t.id, { pct: Number(e.target.value) })} />
                          <span className="ctl-val">{typeof t.pct === "number" ? t.pct : 0}%</span>
                        </div>
                      </div>
                    )}
                  </div>
                  <div className="rung-edit-acts">
                    <button className="mini-ghost" onClick={() => setOpenRung(null)}>Done</button>
                  </div>
                </li>
              )}
              </React.Fragment>
