...
function StatementEditor({ open, book, order, startMonth, currency, savingsPlans = [], debts = [], defaultImportOpen = false, initialWeek = null, initialRange = null, initialBulk = false, initialIds = null, onClose, onSave }) {
            ...
            <div className="se-rows">
              {shownEx.map((r) => (
                <div className="se-row" key={r.id}>
                  ...
                  <select className="se-cat" value={r.debtId ? "debt:" + r.debtId : r.savingsPlanId ? "plan:" + r.savingsPlanId : r.cat}
                    onChange={(e) => {
                      const v = e.target.value;
                      if (v.startsWith("plan:")) editRow("expenses", r.id, { savingsPlanId: v.slice(5), debtId: undefined });
                      else if (v.startsWith("debt:")) editRow("expenses", r.id, { debtId: v.slice(5), savingsPlanId: undefined });
                      else editRow("expenses", r.id, { cat: v, savingsPlanId: undefined, debtId: undefined });
                    }}>
...
function StrategiePanel({ currency, onCurrency, onToast }) {
    ...
    try {
      ...
      if (s) {
        ...
        if (parsed && parsed.statements && parsed.positions) {
          ...
          // schema migration: debts arrived later — seed them if absent/stale
          if (!Array.isArray(merged.debts) || merged.debtSchemaV !== window.STRAT_STATE.debtSchemaV) {
            merged.debts = window.STRAT_STATE.debts;
            merged.debtStrategy = merged.debtStrategy || window.STRAT_STATE.debtStrategy;
            merged.debtSchemaV = window.STRAT_STATE.debtSchemaV;
          }
          return merged;
        }
      }
  ...
  // proper 12-col grid: drag to place, edge-resize, undo — engine in cardgrid.jsx
  const cardLayout = normalizeCardLayout(state.cardLayout, state.cardOrder);
  const cardGrid = useCardGrid(cardLayout, (next) =>
    setState((s) => ({ ...s, cardLayout: next })));
  ...
  const exp = expAll - toPlansMonth - toDebtMonth;
  const surplus = inc - expAll;

  // savings-plan contributions: active month + all time, per plan
  const planMonth = {}, planTotal = {}, debtMonth = {}, debtTotal = {};
  ...
  // spend chart domain: the ENTIRE statement history (contiguous, gaps filled)
  const spendMonths = (() => {
    const keys = [];
    if (sortedKeys.length) {
      let k = sortedKeys[0];
      const last = sortedKeys[sortedKeys.length - 1];
      while (k <= last && keys.length < 120) { keys.push(k); k = window.stepMonth(k, 1); }
    } else {
      keys.push(activeKey);
    }
  ...
  const debtRemaining = (d) => Math.max(0, (d.principal || 0) - (debtTotal[d.id] || 0));
  const openDebtTotal = debts.reduce((s, d) => s + debtRemaining(d), 0);
  const netWorth = positions.invested + positions.emergencySaved - openDebtTotal;
  ...
  // ladder tie-in: step 3 goes live whenever any debt ≥8% APR is still open
  ...
  // every statement row tagged as a debt payment, oldest first — feeds the step-3 detail timeline
  const debtHistory = (() => {
    const out = [];
    for (const [mk, mo] of Object.entries(byMonth)) {
      for (const e of mo.expenses) {
  ...
  const rungLive = (rung) => {
    ...
    return {
      ...
      note: window.fmtMoney(hiOpen, cur, { abbr: true }) + " open above 8% APR — pay down first",
  ...
  const rungRows = (rung) => {
    if (rung.id === 3 && hiOpen > 0) {
      return hiDebts.filter((d) => debtRemaining(d) > 0)
  ...
  const ladderGrand = window.DEFAULT_LADDER.reduce(
    (s, r) => (r.id === 3 && hiOpen > 0 ? s : s + rungRows(r).reduce((a, x) => a + (x.balance || 0), 0)), 0);
  ...
  const onAccountsChange = (dir) => setState((s) => ({ ...s, accountsDirectory: dir }));
    ...
    <div className="strat">
      ...
      <div className="strat-head">
        ...
        <div className="strat-tools">
          ...
          <button className="ghost-btn" onClick={() => setEditorOpen(true)} title="Open the statement editor">
            <IcoFile />
            {Object.keys(statements.byMonth).length > 0 ? "Statement" : "Import statement"}
          </button>
        </div>
      </div>
      ...
      <div className="kpi-row" style={{ marginBottom: 16 }}>
        <div className="kpi">
          ...
          <div className="kpi-sub">Invested + emergency − debt</div>
        </div>
      ...
      {/* main grid */}
      <div className={"strat-grid" + (cardGrid.active ? " is-sorting" : "")} ref={cardGrid.gridRef}>

        {/* hero: projection ↔ spending flip — span 8 */}
        <CardSlot grid={cardGrid} id="hero">
              ...
              <div className={"hero-face hero-back" + (heroFace === "grow" ? " is-hidden" : "")}>
                <div className="proj-figure">
                  <div className="proj-head">
                    ...
                    <div className="proj-cap">monthly spending</div>
                  </div>
                  <div className="proj-split">
                    <div className="split-item">
                    ...
                    {toDebtMonth > 0 && (
                      <div className="split-item" title="Statement rows tagged as debt payments — not counted as spending">
                        {window.fmtMoney(toDebtMonth, cur, { abbr: true })} to debt
                      </div>
                    )}
                  </div>
        ...
        <CardSlot grid={cardGrid} id="ladder">
        <div className="card span-4 ladder-card" ref={ladderRef}>
          ...
          <ol className="ladder" onMouseLeave={() => setRungTip(null)}>
            {window.DEFAULT_LADDER.map((rung0) => {
        ...
        {/* cash flow — span 4 */}
        <CardSlot grid={cardGrid} id="flow">
        ...
        {/* open debt — span 4 */}
        <CardSlot grid={cardGrid} id="debt">
        <DebtCard debts={debts} paidTotal={debtTotal} paidMonth={debtMonth}
          strategy={state.debtStrategy || "avalanche"} activeKey={activeKey} currency={cur}
          onChange={(next) => setState((s) => ({ ...s, debts: next }))}
          onStrategy={(v) => setState((s) => ({ ...s, debtStrategy: v }))}
          onToast={onToast} />
        </CardSlot>

        {/* portfolio: savings programs — span 4 */}
        <CardSlot grid={cardGrid} id="programs">
        <div className="card span-4">
          <div className="card-head">
            <div>
        ...
        <CardSlot grid={cardGrid} id="accounts">
        <AccountsCard accounts={state.accountsDirectory} currency={cur}
          onChange={onAccountsChange} onToast={onToast} />

        </CardSlot>

        {/* savings pots — span 4 */}
        <CardSlot grid={cardGrid} id="pots">
        <div className="card span-4">
          <div className="card-head">
            <div>
        ...
        {/* pillars — span 4 */}
        <CardSlot grid={cardGrid} id="pillars">
        <div className="card span-4">
          <div className="card-head">
            <div>
        ...
        <CardSlot grid={cardGrid} id="cats">
        <div className="card span-8">
          <div className="card-head">
            <div>
      ...
      {detailRung && (
        <LadderDetail
          ...
          debtView={detailRung.id === 3 ? {
            rows: debts.map((d) => ({
              name: d.name,
              tag: (d.rate || 0) > 0 ? d.rate + "% APR" : "interest-free",
