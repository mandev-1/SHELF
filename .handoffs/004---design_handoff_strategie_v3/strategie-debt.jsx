/* ShELF — DebtCard: open liabilities with payoff math + avalanche/snowball ordering.
   Balances are statement-driven, mirroring savings programs: each debt has a
   starting principal; statement rows tagged with debtId pay it down. Months-left
   uses standard amortization from the remaining balance, APR and monthly payment.
   Loaded BEFORE strategie.jsx; exports to window. */

const DbI = {
  pencil: (p) => <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M17 3a2.8 2.8 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg>,
  plus: (p) => <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" {...p}><path d="M12 5v14M5 12h14"/></svg>,
};

function DebtCard({ debts, paidTotal, paidMonth, strategy, activeKey, currency, onChange, onStrategy, onToast }) {
  const { useState } = React;
  const cur = window.CURRENCIES[currency] || window.CURRENCIES.USD;
  const [editId, setEditId] = useState(null);
  const [nName, setNName] = useState("");
  const [nKind, setNKind] = useState("consumer");
  const [nBal, setNBal] = useState("");

  const shown = (base) => Math.round((base || 0) * cur.rate);
  const toBase = (v) => Math.round(v / cur.rate);
  const fmt = (b) => window.fmtMoney(b, currency, { abbr: true });
  ...
  const rows = debts.map((d) => {
    ...
    const mLeft = window.debtMonthsLeft(remaining, pay, d.rate);
    const payoff = mLeft != null && isFinite(mLeft) && mLeft > 0
      ? window.monthAbbr(window.stepMonth(activeKey, mLeft)) : null;
  ...
  const patch = (id, p) => onChange(debts.map((d) => (d.id === id ? { ...d, ...p } : d)));
  const remove = (id) => {
  ...
  const add = () => {
    ...
    onChange([...debts, { id: "dbt" + Date.now(), name, kind: nKind, principal: toBase(bal), rate: 0, payment: 0 }]);
    setNName(""); setNBal("");
    onToast && onToast("Added debt: " + name);
  };
  ...
  return (
    <div className="card span-4">
      <div className="card-head">
        ...
        <div className="seg">
          {[["avalanche", "Avalanche"], ["snowball", "Snowball"]].map(([id, lab]) => (
            <button key={id} className={"seg-btn" + (strategy === id ? " on" : "")}
              title={id === "avalanche" ? "Pay highest APR first — least interest overall" : "Pay smallest balance first — quick wins"}
              onClick={() => onStrategy(id)}>{lab}</button>
          ))}
        </div>
      </div>
      <div className="debt-body">
        ...
        <div className="debt-list">
          {sorted.map((r, i) => {
            const d = r.d;
            const hue = window.debtHue(d);
            const kindLab = (window.DEBT_KINDS.find((k) => k.id === d.kind) || {}).label || d.kind;
            const editing = editId === d.id;
            return (
              <div key={d.id} className="debt">
                <div className="debt-top">
                  <span className="alloc-dot" style={{ background: hue }}></span>
                  <div className="debt-name" title="Double-click to rename"
                    onDoubleClick={() => {
                  ...
                  <button type="button" className={"debt-edit-btn" + (editing ? " on" : "")}
                    title="Edit balance, APR, payment" aria-label={"Edit " + d.name}
                    onClick={() => setEditId(editing ? null : d.id)}><DbI.pencil /></button>
                </div>
                <div className="pot-bar debt-bar"><span style={{ width: r.pct + "%", background: hue }}></span></div>
                ...
                {editing && (
                  <div className="debt-editor">
                    ...
                    <label>APR %
                      <input type="number" step="0.1" min="0" value={d.rate || 0}
                    ...
                    <label>Kind
                      <select value={d.kind} onChange={(e) => patch(d.id, { kind: e.target.value })}>
                        {window.DEBT_KINDS.map((k) => <option key={k.id} value={k.id}>{k.label}</option>)}
                      </select>
                    </label>
            ...
            <div className="debt-cleared">
              {cleared.map((r) => (
                <div key={r.d.id} className="debt-cleared-row">
                  <span className="alloc-dot" style={{ background: window.debtHue(r.d), opacity: 0.4 }}></span>
        ...
        <div className="sp-add">
          <input className="sp-add-name" placeholder="e.g. Car loan — Škoda" value={nName}
            onChange={(e) => setNName(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") add(); }} />
          <select className="se-cat sp-add-kind" value={nKind} onChange={(e) => setNKind(e.target.value)}>
            {window.DEBT_KINDS.map((k) => <option key={k.id} value={k.id}>{k.label}</option>)}
          </select>
          <input className="debt-add-bal" type="text" inputMode="numeric" placeholder="Balance" value={nBal}
            onChange={(e) => setNBal(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") add(); }} />
          <button className="ghost-btn" onClick={add} disabled={!nName.trim() || !nBal.trim()}><DbI.plus /> Add</button>
        </div>
      </div>
    </div>
  );
