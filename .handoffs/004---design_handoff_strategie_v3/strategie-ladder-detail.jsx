      ...
      <div className="ld-modal" role="dialog" aria-modal="true" aria-label={rung.title}
        ...
        <div className="ld-body">
          ...
          <div className="ld-sec-head">
            <h3 className="ld-sec-title">{debtView ? "What's still owed" : "Where the money sits"}</h3>
            <div className="ld-sec-actions">
              {!editing && displayRows.length > 0 && <span className="ld-sec-total">{f(total)}</span>}
              {debtView ? null : !editing ? (
          ...
          {!editing && (
            displayRows.length > 0 ? (
              <div className="ld-accts">
                ...
                {debtView && (
                  <div className="ld-debt-hint">
                    Balances update automatically from statement rows tagged as debt payments.
                    Edit debts on the <em>Open debt</em> card.
                  </div>
                )}
          ...
          <div className="ld-sec-head ld-sec-head--mt">
            <h3 className="ld-sec-title">{debtView ? "Payment history" : "History"}</h3>
            {history.length > 0 && (
              <span className="ld-sec-sub">{history.length} {debtView ? (history.length === 1 ? "payment" : "payments") : "moves"}</span>
            )}
          </div>
          ...
          {history.length > 0 ? (
            ...
            <div className="ld-empty">{debtView ? "No payments tagged yet — mark statement rows as debt payments and they'll show up here." : "No moves recorded yet."}</div>
          )}
        </div>
      </div>
    </div>
