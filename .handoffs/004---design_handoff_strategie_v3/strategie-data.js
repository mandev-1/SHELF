...
function debtHue(d) {
  const k = DEBT_KINDS.find((x) => x.id === (d && d.kind));
  return (k || DEBT_KINDS[DEBT_KINDS.length - 1]).hue;
}
/** standard amortization: months to clear `balance` at `apr`% paying `payment`/mo.
    null = no payment set; Infinity = payment doesn't cover interest. */
function debtMonthsLeft(balance, payment, apr) {
...
function _buildMonth(key, v) {
  ...
  const expenses = [
    ...
    { id: "exp-car-" + key, label: "Car loan payment",    amt: 260, cat: "other", date: dayStr(key, 15), debtId: "dbt-car" },
    { id: "exp-cc-" + key,  label: "Credit card payment", amt: 120, cat: "other", date: dayStr(key, 20), debtId: "dbt-cc" },
  ]
...
Object.assign(window, {
  ...
  DEBT_KINDS, debtHue, debtMonthsLeft,
