...
function Favicon({ host, cls = "bm-fav" }) {
  ...
  return (
    <span className={cls}>
      {err
        ? <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)" }}>{(host || "?")[0].toUpperCase()}</span>
      ...
      <div className="pillar-body">
        <div className="zone">
          ...
          <div className="pin-stack">
            {data.pins.map((p) => (
              <div className="pin" key={p.id} draggable>
...
function FolderCard({ folder }) {
  ...
  return (
    <div className="folder" style={{ "--hue": folder.hue }}>
      <div className="folder-head">
        <span className="folder-title">{folder.title}</span>
        {folder.dot ? <span className="folder-dot" /> : <span className="folder-count">{count}</span>}
      </div>
