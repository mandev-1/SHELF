...
const SI = {
  ...
  flip: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M3 7a9 9 0 0 1 15-3l3 3"/><path d="M21 5v4h-4"/><path d="M21 17a9 9 0 0 1-15 3l-3-3"/><path d="M3 19v-4h4"/></svg>,
  shield: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M12 3 5 6v5c0 4.4 3 8 7 9 4-1 7-4.6 7-9V6z"/></svg>,
  gift: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><rect x="4" y="10" width="16" height="10" rx="1.5"/><path d="M4 10V8h16v2M12 10v10M12 10c-2 0-4-1-4-3a2 2 0 0 1 4 0c0 2 2 3 4 3a2 2 0 0 0-4-3"/></svg>,
  flame: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M12 3c1 3 4 4 4 8a4 4 0 0 1-8 0c0-2 1-3 1-3 .5 1.5 1.5 2 1.5 2C10 8 11 5 12 3Z"/></svg>,
  vault: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="11" cy="12" r="4"/><path d="M11 8v.5M11 16v-.5M15.5 12H15M7 12h.5M18 8v8"/></svg>,
  leaf: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M5 19c0-8 6-13 14-13 0 8-5 14-13 14M5 19c2-4 4-6 8-8"/></svg>,
  growth: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M4 19 10 12l3.5 3.5L20 7"/><path d="M15 7h5v5"/></svg>,
  target: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="4"/><circle cx="12" cy="12" r="0.6" fill="currentColor"/></svg>,
  ...
  repeat: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M17 2l3 3-3 3"/><path d="M4 11V9a4 4 0 0 1 4-4h12"/><path d="M7 22l-3-3 3-3"/><path d="M20 13v2a4 4 0 0 1-4 4H4"/></svg>,
  pencil: (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>,
};
...
/* ---------- the spending trend chart (monthly spend over time) ---------- */
function SpendingChart({ series, avg, currency }) {
  const W = 760, H = 300, ml = 12, mr = 16, mt = 16, mb = 24;
  const plotW = W - ml - mr, plotH = H - mt - mb;
  const n = Math.max(1, series.length - 1);
  const max = niceCeil(Math.max(1, ...series.map((s) => s.total)));
  const x = (i) => ml + (i / n) * plotW;
  const y = (v) => mt + (1 - v / max) * plotH;
  const base = mt + plotH;

  const linePath = series.map((s, i) => `${i ? "L" : "M"} ${x(i).toFixed(1)} ${y(s.total).toFixed(1)}`).join(" ");
  const areaPath = `${linePath} L ${x(series.length - 1).toFixed(1)} ${base} L ${ml} ${base} Z`;
  const gridVals = [0, 0.25, 0.5, 0.75, 1].map((f) => f * max);

  return (
    <svg className="proj-svg" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" role="img" aria-label="Spending trend">
      {gridVals.map((v, i) => (
        <g key={i}>
          <line x1={ml} x2={W - mr} y1={y(v)} y2={y(v)} className="proj-grid" />
          <text x={W - mr} y={y(v) - 4} className="proj-ylab" textAnchor="end">{window.fmtMoney(v, currency, { abbr: true })}</text>
      ...
      {avg > 0 && (
        <g>
          <line x1={ml} x2={W - mr} y1={y(avg)} y2={y(avg)} className="spend-avgline" />
          <text x={ml} y={y(avg) - 5} className="spend-avglab">avg {window.fmtMoney(avg, currency, { abbr: true })}</text>
        </g>
      )}
      <path d={areaPath} className="spend-area" />
      <path d={linePath} className="spend-line" />
      {series.map((s, i) => (
        <circle key={i} cx={x(i)} cy={y(s.total)} r={i === series.length - 1 ? 4.5 : 2.6}
          className={i === series.length - 1 ? "spend-dot-end" : "spend-dot"} />
      ))}
      {series.map((s, i) => (
        <text key={"l" + i} x={x(i)} y={H - 6} className="proj-xlab"
          textAnchor={i === 0 ? "start" : i === series.length - 1 ? "end" : "middle"}>{s.label}</text>
      ))}
    </svg>
  );
}
...
function StatementEditor({ open, book, order, startMonth, currency, onClose, onSave }) {
  ...
  const [pickerOpen, setPickerOpen] = useState(false); // month picker popover
  const pickerRef = useRef(null);
  const [pausedMem, setPausedMem] = useState(() => new Set()); // memberships toggled off
  const [members, setMembers] = useState(() => (window.STRAT.memberships || []).map((m) => ({ ...m })));
  const [memEditor, setMemEditor] = useState(null); // null | { mode:"add"|"edit", id?, name, plan, price, color, mono }
  ...
  // close the month picker on outside click
  useEffect(() => {
  ...
  useEffect(() => {
    if (!open) return;
    const h = (e) => { if (e.key === "Escape") { if (pickerOpen) { setPickerOpen(false); } else { onClose(); } } };
  ...
  // jump straight to any month from the picker
  const jumpTo = (k) => { setViewKey(k); setPickerOpen(false); };
  // add a fresh month after the last one, seeded from it
  const addMonthAtEnd = () => {
  ...
  // net surplus for any month key (USD base)
  const netOf = (k) => {
    const mo = draft[k] || { income: [], expenses: [] };
    return mo.income.reduce((a, r) => a + (r.amt || 0), 0) - mo.expenses.reduce((a, r) => a + (r.amt || 0), 0);
  };
      ...
      <div className="se-modal" role="dialog" aria-modal="true" aria-label="Edit statement">
        ...
        <div className="se-periodbar">
          <div className="se-stepper">
            ...
            <div className="se-monthpick" ref={pickerRef}>
              <button className={"se-month se-month--btn" + (pickerOpen ? " open" : "")}
                onClick={() => setPickerOpen((o) => !o)} aria-haspopup="listbox" aria-expanded={pickerOpen}>
                <span className="se-month-name">{monthLabel(viewKey)}<SI.chev className="se-month-caret" /></span>
              ...
              {pickerOpen && (
                <div className="se-monthmenu" role="listbox" aria-label="Pick a month">
                  {keys.map((k) => {
                    const net = netOf(k);
                    return (
                      <button key={k} role="option" aria-selected={k === viewKey}
                        className={"se-mo-opt" + (k === viewKey ? " on" : "")} onClick={() => jumpTo(k)}>
                        <span className="se-mo-check">{k === viewKey ? <SI.check /> : null}</span>
                        <span className="se-mo-name">{monthLabel(k)}</span>
                        <span className={"se-mo-net" + (net < 0 ? " neg" : "")}>{net < 0 ? "−" : "+"}{fmt(Math.abs(net))}</span>
                      </button>
                    );
                  })}
                  <button className="se-mo-opt se-mo-add" onClick={addMonthAtEnd}>
                    <span className="se-mo-check"><SI.plus /></span>
        ...
        <div className="se-body">
          ...
          <section className="se-col se-col--in">
            ...
            {/* memberships — the recurring subscriptions, as little tiles */}
            {(() => {
              const PALETTE = ["#E50914", "#FF6B2C", "#F59E0B", "#1DB954", "#10A37F", "#3B82F6", "#1A47BE", "#8B5CF6", "#EC4899", "#8E8E93"];
              const initials = (name) => (name || "").trim().split(/\s+/).map((w) => w[0]).join("").slice(0, 2).toUpperCase();
              const activeMems = members.filter((m) => !pausedMem.has(m.id));
              const memTotal = activeMems.reduce((a, m) => a + m.price, 0);
              const toggle = (id) => setPausedMem((s) => {
                const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n;
              });
              const openAdd = () => setMemEditor({ mode: "add", name: "", plan: "", price: "", color: PALETTE[5], mono: "", monoTouched: false });
              const openEdit = (m) => setMemEditor({ mode: "edit", id: m.id, name: m.name, plan: m.plan || "", price: String(shown(m.price)), color: m.color, mono: m.mono, monoTouched: true });
              const upd = (patch) => setMemEditor((e) => ({ ...e, ...patch }));
              const save = () => {
                const ed = memEditor;
                const name = ed.name.trim() || "Untitled";
                const mono = (ed.monoTouched && ed.mono.trim()) ? ed.mono.trim().slice(0, 2).toUpperCase() : (initials(name) || "?");
                const price = toBase(parseInt(String(ed.price).replace(/[^\d]/g, ""), 10) || 0);
                if (ed.mode === "add") {
                  setMembers((m) => [...m, { id: "m_" + Date.now(), name, plan: ed.plan.trim(), price, color: ed.color, mono }]);
                } else {
                  setMembers((m) => m.map((x) => x.id === ed.id ? { ...x, name, plan: ed.plan.trim(), price, color: ed.color, mono } : x));
                }
                setMemEditor(null);
              };
              const remove = () => {
                setMembers((m) => m.filter((x) => x.id !== memEditor.id));
                setPausedMem((s) => { const n = new Set(s); n.delete(memEditor.id); return n; });
                setMemEditor(null);
              };
              const previewMono = (memEditor && ((memEditor.monoTouched && memEditor.mono.trim()) ? memEditor.mono.trim().slice(0, 2).toUpperCase() : initials(memEditor.name))) || "?";
              return (
                <div className="se-mem">
                  <div className="se-mem-head">
                    <span className="se-mem-title">
                      <span className="se-pill se-pill--mem"><SI.repeat /></span>
                    ...
                    {members.map((m) => {
                      ...
                      return (
                        <button key={m.id} type="button"
                          ...
                          onClick={() => toggle(m.id)}
                          aria-pressed={!off}
                          aria-label={m.name + " · " + (m.plan || "") + (off ? " (paused)" : "")}>
                          <span className="se-tile-mono">{m.mono}</span>
                          <span className="se-tile-edit" role="button" tabIndex={-1}
                            onClick={(e) => { e.stopPropagation(); openEdit(m); }} aria-label={"Edit " + m.name}>
                            <SI.pencil />
                          </span>
                          <span className="se-tile-tip">
                            <b>{m.name}</b>{m.plan ? <i>{m.plan}</i> : null}<em>{fmt(m.price)}/mo</em>
                          </span>
                        </button>
                      );
                    })}
                    <button type="button" className="se-tile se-tile--add" aria-label="Add membership" onClick={openAdd}>
                      <SI.plus />
                    </button>
                  </div>
                  ...
                  {memEditor && (
                    <div className="se-mem-editor">
                      <div className="se-mem-ed-top">
                        <span className="se-tile se-tile--preview" style={{ "--brand": memEditor.color }}>
                          <span className="se-tile-mono">{previewMono}</span>
                        </span>
                        <div className="se-mem-ed-fields">
                          <input className="se-mem-in" placeholder="Name (e.g. Netflix)" value={memEditor.name} autoFocus
                            onChange={(e) => upd({ name: e.target.value })} />
                          <input className="se-mem-in se-mem-in--sub" placeholder="Plan (optional)" value={memEditor.plan}
                      ...
                      <div className="se-mem-ed-row">
                        <label className="se-mem-field">
                          ...
                          <span className="se-mem-price"><i>{cur.code}</i>
                            <input inputMode="numeric" value={memEditor.price} placeholder="0"
                        ...
                        <label className="se-mem-field se-mem-field--mono">
                          ...
                          <input className="se-mem-in se-mem-mono-in" maxLength={2} value={memEditor.mono}
                            placeholder={initials(memEditor.name) || "AB"}
                      ...
                      <div className="se-mem-swatches">
                        {PALETTE.map((c) => (
                          <button key={c} type="button"
                            ...
                            style={{ "--brand": c }} onClick={() => upd({ color: c })} aria-label={"Color " + c} />
                        ))}
                      </div>
                      <div className="se-mem-ed-actions">
                        {memEditor.mode === "edit"
                          ? <button type="button" className="se-mem-btn se-mem-btn--del" onClick={remove}><SI.trash /> Remove</button>
                          : <span />}
                        <div className="se-mem-ed-right">
                          <button type="button" className="se-mem-btn" onClick={() => setMemEditor(null)}>Cancel</button>
                          <button type="button" className="se-mem-btn se-mem-btn--save" onClick={save}>
                  ...
                  <div className="se-mem-foot">
                    <span>{activeMems.length} active{pausedMem.size ? ` · ${pausedMem.size} paused` : ""}</span>
...
/* ---------- ladder step detail (opens when you click a rung) ---------- */
function LadderDetail({ step, currency, onClose }) {
  ...
  const f = (v) => window.fmtMoney(v, currency, { abbr: false });
  const Icon = SI[step.icon] || SI.check;
  const statusLabel = step.status === "done" ? "Funded" : step.status === "active" ? "In progress" : "Not started yet";

  const total = (step.accounts || []).reduce((a, x) => a + (x.balance || 0), 0);
  const pctOfTarget = step.target ? Math.min(100, Math.round(total / step.target * 100)) : null;
  ...
  return (
    <div className="ld-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="ld-modal" role="dialog" aria-modal="true" aria-label={step.title} style={{ "--step-hue": step.hue }}>
        {/* hero header */}
        <div className="ld-hero">
          <div className="ld-hero-wash" aria-hidden="true"></div>
          <Icon className="ld-hero-ghost" aria-hidden="true" />
          <button className="ld-close" onClick={onClose} aria-label="Close"><SI.x /></button>
          <div className="ld-hero-row">
            <span className="ld-hero-badge"><Icon /></span>
            <span className={"ld-status ld-status--" + step.status}>
              {step.status === "done" ? <SI.check /> : step.status === "queued" ? <SI.lock /> : <SI.clock />}
              {statusLabel}{step.status === "active" && typeof step.pct === "number" ? ` · ${step.pct}%` : ""}
            </span>
          </div>
          <div className="ld-hero-foot">
            <span className="ld-step">Step {step.id} of 7</span>
            <h2 className="ld-title">{step.title}</h2>
          </div>
        </div>

        {/* body */}
        <div className="ld-body">
          <p className="ld-blurb">{step.blurb}</p>

          {/* where the money sits */}
          <div className="ld-sec-head">
            <h3 className="ld-sec-title">Where the money sits</h3>
            {step.accounts && step.accounts.length > 0 && <span className="ld-sec-total">{f(total)}</span>}
          </div>

          {step.accounts && step.accounts.length > 0 ? (
            <div className="ld-accts">
              {step.accounts.map((a, i) => (
                <div className="ld-acct" key={i}>
                  ...
                  <span className="ld-acct-meta">
                    ...
                    <span className="ld-acct-tag">{a.tag}</span>
                  </span>
              ...
              {pctOfTarget !== null && (
                <div className="ld-target">
                  ...
                  <div className="ld-target-foot">
                    ...
                    <span>{f(total)} / {f(step.target)}</span>
                  </div>
                </div>
              )}
            </div>
          ) : (
            <div className="ld-empty">Nothing here yet — this step unlocks once the earlier ones are funded.</div>
          )}

          {/* history */}
          <div className="ld-sec-head ld-sec-head--mt">
            <h3 className="ld-sec-title">History</h3>
            {step.history && step.history.length > 0 && <span className="ld-sec-sub">{step.history.length} moves</span>}
          </div>

          {step.history && step.history.length > 0 ? (
            <ol className="ld-time">
              {[...step.history].reverse().map((h, i) => (
                <li className="ld-event" key={i}>
                  <span className="ld-event-node"></span>
                  <span className="ld-event-date">{fmtDate(h.date)}</span>
...
/* ---------- main panel ---------- */
function StrategiePanel({ currency, onCurrency, onToast, extraAssets = 0 }) {
  ...
  const [openStep, setOpenStep] = useState(null); // ladder rung whose detail is open
  ...
  const netWorth = S.positions.invested + S.positions.emergencySaved + potsSaved + (extraAssets || 0);
  ...
  // ---- monthly spending series across the statement book (for the flip chart) ----
  const spendSeries = useMemo(() => {
    const keys = (order && order.length ? order : Object.keys(book)).slice().sort();
    return keys.map((k) => ({
      key: k,
      label: monthAbbr(k).replace(/ \d+$/, ""),
      total: (book[k]?.expenses || []).reduce((a, r) => a + (r.amt || 0), 0),
    }));
  }, [book, order]);
  const spendAvg = spendSeries.length ? spendSeries.reduce((a, s) => a + s.total, 0) / spendSeries.length : 0;
  const activeSpend = (activeStmt.expenses || []).reduce((a, r) => a + (r.amt || 0), 0);
  const spendPrev = spendSeries.length > 1 ? spendSeries[spendSeries.length - 2].total : 0;
  const spendDelta = activeSpend - spendPrev;
  const [heroFace, setHeroFace] = useState("grow");   // grow | spend
  const [heroFlipping, setHeroFlipping] = useState(false);
  const heroTimer = React.useRef(null);
  const flipHero = () => {
    setHeroFace((p) => (p === "grow" ? "spend" : "grow"));
    setHeroFlipping(true);
    clearTimeout(heroTimer.current);
    heroTimer.current = setTimeout(() => setHeroFlipping(false), 640);
  };
  React.useEffect(() => () => clearTimeout(heroTimer.current), []);
  ...
  return (
    <div className="strat">
      ...
      <LadderDetail step={openStep} currency={currency} onClose={() => setOpenStep(null)} />
      ...
      <div className="kpi-row">
        <div className="kpi">
          ...
          <div className="kpi-sub up">▲ invested + reserves{extraAssets ? " + inventory" : ""}</div>
        </div>
        <div className="kpi">
      ...
      <div className="strat-grid">
        {/* projection (hero) — flips between the compounding engine and spending */}
        <section className="card span-8 hero-card">
          <div className="card-head">
            <div>
              <div className="card-eyebrow">{heroFace === "grow" ? "Compounding engine" : monthLabel(activeMonth) + " · spending trend"}</div>
              <h3 className="card-title">{heroFace === "grow" ? `If you invest ${f(monthly)}/mo at ${(ret.rate * 100).toFixed(0)}%` : "Where the money goes"}</h3>
            </div>
            <div className="hero-head-r">
              ...
              <button className={"hero-flip-btn" + (heroFace === "spend" ? " on" : "")}
                onClick={flipHero}
                title={heroFace === "grow" ? "Flip to spending" : "Flip to growth"} aria-label="Flip chart">
                <SI.flip />
              </button>
            </div>
          </div>
          ...
          <div className="hero-scene">
            <div className={"hero-stack" + (heroFace === "spend" ? " flipped" : "")}>
              {/* FRONT — compounding engine */}
              <div className={"hero-face hero-front" + (heroFace !== "grow" && !heroFlipping ? " is-hidden" : "")} aria-hidden={heroFace !== "grow"}>
              ...
              {/* BACK — spending trend */}
              <div className={"hero-face hero-back" + (heroFace !== "spend" && !heroFlipping ? " is-hidden" : "")} aria-hidden={heroFace !== "spend"}>
                <div className="proj-figure">
                  <div className="proj-head">
                    <div>
                      <div className="proj-big">{f(activeSpend)}</div>
                      <div className="proj-cap">spent in {monthLabel(activeMonth)}</div>
                    </div>
                    <div className="proj-split">
                      <div className="split-item"><span className="sw sw-spend"></span>This month <b>{f(activeSpend)}</b></div>
                      <div className="split-item">
                        <span className={"split-delta" + (spendDelta > 0 ? " up" : " down")}>{spendDelta > 0 ? "▲" : "▼"} {f(Math.abs(spendDelta), { abbr: true })}</span> vs last
                      </div>
                    </div>
                  </div>

                  <SpendingChart series={spendSeries} avg={spendAvg} currency={currency} />

                  <div className="proj-controls proj-controls--spend">
                    <button className="mini-ghost" onClick={() => setEditorOpen(true)}><SI.file style={{ width: 13, height: 13 }} /> Edit statement</button>
                    <span className="spend-foot-note">Monthly average <b>{f(spendAvg)}</b> across {spendSeries.length} months</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <ol className="ladder">
            {S.ladder.map((t) => (
              <li key={t.id} className={"rung rung--" + t.status}>
                <button className="rung-hit" onClick={() => setOpenStep(t)} aria-label={`Open ${t.title} details`}>
                  ...
                  <SI.chev className="rung-chev" style={{ transform: "rotate(-90deg)" }} />
                </button>
              </li>
            ))}
          </ol>
