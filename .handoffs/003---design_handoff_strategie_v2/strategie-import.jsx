...
function StatementImport({ currency, existing, savingsPlans = [], editRows, onApplyEdits, onClose, onImport }) {
  ...
  const applyBulk = () => {
    ...
    setRows((rs) => rs.map((r) => {
      ...
      if (bulkCat) {
        ...
        else { patch.cat = bulkCat; patch.savingsPlanId = undefined; }
      }
      return { ...r, ...patch };
    }));
                            ...
                            <React.Fragment>
                              ...
                              <select className="se-cat si-cat"
                                ...
                                onChange={(e) => {
                                  ...
                                  else setRow(r.key, { cat: v, savingsPlanId: undefined });
                                }}>
                                {window.CAT_KEYS_BY_LABEL.map((k) => <option key={k} value={k}>{window.STMT_CATS[k].label}</option>)}
                                {savingsPlans.length > 0 && (
                                  <optgroup label="Savings plans">
